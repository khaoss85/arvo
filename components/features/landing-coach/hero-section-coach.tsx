...
interface HeroSectionCoachProps {
  isAuthenticated: boolean;
  showWaitlist?: boolean;
}

export function HeroSectionCoach({ isAuthenticated, showWaitlist = false }: HeroSectionCoachProps) {
  const t = useTranslations('landing.forTrainers.hero');
        ...
        <div className="flex flex-col items-center text-center space-y-8">
          ...
          >
            <div className="px-4 py-2 rounded-full bg-primary-50 dark:bg-primary-900/50 text-primary-700 dark:text-primary-200 border border-primary-200 dark:border-primary-800">
              ...
              <span className="font-bold">{t('stats.timeSavedValue')}</span> {t('stats.timeSaved')}
            </div>
            <div className="px-4 py-2 rounded-full bg-primary-50 dark:bg-primary-900/50 text-primary-700 dark:text-primary-200 border border-primary-200 dark:border-primary-800">
              <Users className="inline-block w-4 h-4 mr-1" />
              <span className="font-bold">{t('stats.clientCapacityValue')}</span> {t('stats.clientCapacity')}
            </div>
            <div className="px-4 py-2 rounded-full bg-primary-50 dark:bg-primary-900/50 text-primary-700 dark:text-primary-200 border border-primary-200 dark:border-primary-800">
              <Heart className="inline-block w-4 h-4 mr-1" />
              <span className="font-bold">{t('stats.satisfactionValue')}</span> {t('stats.satisfaction')}
            </div>
          </motion.div>
          ...
          >
            <Link href={isAuthenticated ? "/dashboard" : (showWaitlist ? "/waitlist" : "/login")}>
              <Button size="lg" className="text-base px-8 h-12 group">
                {isAuthenticated ? "Go to Dashboard" : (showWaitlist ? "Join the Waitlist" : t('cta.primary'))}

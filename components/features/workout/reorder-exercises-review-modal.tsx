...
interface ReorderExercisesReviewModalProps {
  exercises: Exercise[]
  workoutId: string
  workoutType: string
  approachId: string
...
export function ReorderExercisesReviewModal({
  exercises,
  workoutId,
  workoutType,
  approachId,
  ...
  const handleValidate = async () => {
    setValidating(true)
    try {
      const originalOrder = exercises.map(ex => ex.name)
      const newOrder = orderedExercises.map(ex => ex.name)
  ...
  const handleApply = async () => {
    ...
    try {
      // Save to database
      const result = await updateWorkoutExercisesAction(workoutId, orderedExercises)
      ...
      onReorderComplete(orderedExercises)

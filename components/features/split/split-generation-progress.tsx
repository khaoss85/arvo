...
interface SplitGenerationProgressProps {
  requestId: string
  onComplete?: (splitPlanId: string) => void
...
/**
 * SplitGenerationProgress Component
 *
 * Shows real-time progress for asynchronous split generation
 *
 * Features:
 ...
 * - Auto-redirects to dashboard when ready (optional)
 * - Handles errors with retry option
 */
export function SplitGenerationProgress({
  requestId,
  onComplete,
  onError,
  onClose,
  autoRedirect = true,
}: SplitGenerationProgressProps) {
  const t = useTranslations('split.generation')
  ...
  const pollStatus = useCallback(async () => {
    try {
      const response = await fetch(`/api/splits/generation-status/${requestId}`)
      ...
      if (data.status === 'completed' && data.split_plan_id) {
        setSplitPlanId(data.split_plan_id)
        onComplete?.(data.split_plan_id)

        // Auto-redirect to dashboard if enabled
        if (autoRedirect) {
          setTimeout(() => {
      ...
      console.error('[SplitGenProgress] Polling error:', error)
  ...
  const handleRetry = () => {
    // Redirect to onboarding/review to retry
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            {status === 'completed' && !autoRedirect && splitPlanId && (
              <span>{t('ready')}</span>
            )}
            {status === 'failed' && (
        ...
        <div className="space-y-4 py-4">
          ...
          {/* Success state */}
          {status === 'completed' && splitPlanId && !autoRedirect && (
            <Button
